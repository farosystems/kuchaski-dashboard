-- ============================================================================
-- SCRIPT DE MIGRACIÓN COMPLETO - CATÁLOGO KUCHASKI
-- ============================================================================
-- Sistema de catálogo de productos con financiación, combos y promociones
-- Fecha de generación: 2026-01-27
-- ============================================================================
--
-- INSTRUCCIONES DE USO:
-- 1. Crear una nueva base de datos PostgreSQL vacía
...
-- ============================================================================
-- 1. LIMPIEZA PREVIA (OPCIONAL - DESCOMENTAR SI NECESITAS LIMPIAR)
-- ============================================================================

-- ADVERTENCIA: Esto eliminará todas las tablas existentes
-- Descomentar solo si estás seguro de limpiar la base de datos

-- DROP TABLE IF EXISTS public.promo_productos CASCADE;
...
-- DROP TABLE IF EXISTS public.productos_planes_default CASCADE;
-- DROP TABLE IF EXISTS public.productos_planes CASCADE;
-- DROP TABLE IF EXISTS public.planes_categorias CASCADE;
-- DROP TABLE IF EXISTS public.configuracion_zonas CASCADE;
-- DROP TABLE IF EXISTS public.configuracion_web CASCADE;
-- DROP TABLE IF EXISTS public.configuracion CASCADE;
-- DROP TABLE IF EXISTS public.zonas CASCADE;
-- DROP TABLE IF EXISTS public.productos CASCADE;
-- DROP TABLE IF EXISTS public.planes_financiacion CASCADE;
...
CREATE TABLE public.productos (
    ...
    descuento_porcentual numeric(5,2) default 0 check (descuento_porcentual >= 0 and descuento_porcentual <= 100),
    precio_oferta numeric(10,2) check (precio_oferta >= 0),
    fecha_vigencia_desde timestamp with time zone null,
    fecha_vigencia_hasta timestamp with time zone null,
    fk_id_categoria bigint null,
    fk_id_marca bigint null,

    constraint productos_pkey primary key (id),
    constraint productos_descuento_fechas_validas check (fecha_vigencia_hasta is null or fecha_vigencia_desde is null or fecha_vigencia_hasta > fecha_vigencia_desde)
);

COMMENT ON TABLE public.productos IS 'Catálogo principal de productos con múltiples imágenes y sistema de descuentos';
...
-- ----------------------------------------------------------------------------
-- 3.8 TABLA: productos_planes_default
...
-- ----------------------------------------------------------------------------
-- 3.11 TABLA: configuracion_zonas
...
-- ----------------------------------------------------------------------------
-- 5.1 TABLA: promos
-- Descripción: Tabla de promociones con descuentos por porcentaje
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 5.2 TABLA: promo_productos
-- Descripción: Relación muchos-a-muchos entre promociones y productos
-- ----------------------------------------------------------------------------
CREATE TABLE public.promo_productos (
    id bigint generated by default as identity primary key,
...
-- ============================================================================
-- 6. FOREIGN KEYS (RELACIONES ENTRE TABLAS)
-- ============================================================================
...
-- Relaciones de promo_productos
ALTER TABLE public.promo_productos
    ADD CONSTRAINT promo_productos_promo_id_fkey
    FOREIGN KEY (promo_id)
    REFERENCES public.promos(id) ON DELETE CASCADE;

ALTER TABLE public.promo_productos
    ADD CONSTRAINT promo_productos_producto_id_fkey
...
CREATE INDEX productos_descuento_porcentual_idx ON public.productos (descuento_porcentual) WHERE descuento_porcentual > 0;
CREATE INDEX productos_precio_oferta_idx ON public.productos (precio_oferta) WHERE precio_oferta IS NOT NULL;
CREATE INDEX productos_fecha_vigencia_desde_idx ON public.productos (fecha_vigencia_desde) WHERE fecha_vigencia_desde IS NOT NULL;
CREATE INDEX productos_fecha_vigencia_hasta_idx ON public.productos (fecha_vigencia_hasta) WHERE fecha_vigencia_hasta IS NOT NULL;
...
-- Índices para promos
CREATE INDEX promos_fecha_inicio_idx ON public.promos(fecha_inicio);
CREATE INDEX promos_fecha_fin_idx ON public.promos(fecha_fin);
CREATE INDEX promos_activa_idx ON public.promos(activa);

-- Índices para promo_productos
CREATE INDEX promo_productos_promo_id_idx ON public.promo_productos(promo_id);
CREATE INDEX promo_productos_producto_id_idx ON public.promo_productos(producto_id);
...
-- ----------------------------------------------------------------------------
-- 7.2 FUNCIÓN: calcular_precio_combo
...
-- ----------------------------------------------------------------------------
-- 7.3 FUNCIÓN: trigger_actualizar_precio_combo
...
-- ----------------------------------------------------------------------------
-- 7.4 FUNCIÓN: trigger_actualizar_combos_por_producto
...
-- ----------------------------------------------------------------------------
-- 7.5 FUNCIÓN: trigger_actualizar_precio_por_descuento
...
-- Trigger para actualizar updated_at en promos
...
CREATE POLICY "Permitir acceso completo a promos"
    ON public.promos FOR ALL USING (true);

-- Políticas para promo_productos
CREATE POLICY "Permitir acceso completo a promo_productos"
    ON public.promo_productos FOR ALL USING (true);

-- ============================================================================
-- 12. STORAGE BUCKETS (SUPABASE - OPCIONAL)
-- ============================================================================

-- NOTA: Esta sección solo aplica si estás usando Supabase Storage
...
-- Insertar configuración web por defecto (solo si no existe)
...
-- Ejecutar después de correr el script completo

SELECT
    table_schema,
    table_name,
    (SELECT COUNT(*) FROM information_schema.columns c WHERE c.table_name = t.table_name AND c.table_schema = t.table_schema) as columnas
...
-- 27 Foreign Keys (relaciones)
-- 45+ Índices para optimización
-- 5 Funciones PL/pgSQL
-- 6 Triggers automáticos
-- 16 Políticas RLS (Row Level Security)
-- 4 Políticas de Storage (Supabase)
--
-- JERARQUÍA DE PRODUCTOS:
-- Líneas > Categorías > Productos
--
-- SISTEMA DE FINANCIACIÓN:
-- - Planes flexibles con múltiples configuraciones
-- - Asociación automática por categorías
-- - Asociación manual por productos
-- - Soporte para anticipos y recargos
--
-- SISTEMA DE COMBOS:
...
-- - Asociación N:N con productos
-- - Activación/desactivación manual
--
-- SISTEMA DE DESCUENTOS EN PRODUCTOS:
-- - Descuento porcentual configurable
-- - Precio de oferta alternativo
-- - Fechas de vigencia personalizables
-- - Validaciones automáticas
--
-- CARACTERÍSTICAS ESPECIALES:
-- - Múltiples imágenes por producto (5) y combo (5)
-- - Configuración web responsive (desktop/mobile)
-- - Gestión de stock
-- - Productos destacados
-- - Configuración por zonas geográficas
-- - Banners personalizables (3 banners)
